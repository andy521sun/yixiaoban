...
class WebSocketService extends ChangeNotifier {
  ...
  void _setupEventListeners() {
    ...
    _socket!.onConnect((_) {
      ...
      // 触发连接成功事件
      _emitEvent(WsEvents.connectionEstablished, {
  ...
  void _setupSpecificEventListeners() {
    ...
    // 聊天消息
    _socket!.on(WsEvents.chatMessage, (data) {
      _handleChatMessage(data);
    });
    
    // 消息已发送回执
    _socket!.on(WsEvents.messageSent, (data) {
      _emitEvent(WsEvents.messageSent, data);
    });
    
    // 消息已读回执
    _socket!.on(WsEvents.messageRead, (data) {
      _emitEvent(WsEvents.messageRead, data);
    });
    
    // 输入状态
    _socket!.on(WsEvents.typing, (data) {
      _emitEvent(WsEvents.typing, data);
    });
    
    // 通话请求
    _socket!.on(WsEvents.callRequest, (data) {
      _emitEvent(WsEvents.callRequest, data);
    });
    
    // 通话响应
    _socket!.on(WsEvents.callResponse, (data) {
      _emitEvent(WsEvents.callResponse, data);
    });
    
    // 通话建立
    _socket!.on(WsEvents.callEstablished, (data) {
      _emitEvent(WsEvents.callEstablished, data);
    });
    
    // 用户状态更新
    _socket!.on(WsEvents.userStatus, (data) {
      _emitEvent(WsEvents.userStatus, data);
    });
    
    // 订单通知
    _socket!.on(WsEvents.orderNotification, (data) {
      _emitEvent(WsEvents.orderNotification, data);
    });
    
    // 系统通知
    _socket!.on(WsEvents.systemNotification, (data) {
      _emitEvent(WsEvents.systemNotification, data);
    });
    
    // 错误
    _socket!.on(WsEvents.error, (data) {
      _emitEvent(WsEvents.error, data);
    });
    
    // 心跳响应
    _socket!.on(WsEvents.pong, (data) {
      _emitEvent(WsEvents.pong, data);
    });
  }
  ...
  void _handleChatMessage(dynamic data) {
    _emitEvent(WsEvents.chatMessage, data);
  }
