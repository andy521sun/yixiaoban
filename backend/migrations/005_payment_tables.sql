...
CREATE TABLE IF NOT EXISTS user_wallets (
    ...
    security_answer_hash VARCHAR(255) COMMENT '安全答案哈希',
    last_recharge_time TIMESTAMP NULL COMMENT '最后充值时间',
    last_withdraw_time TIMESTAMP NULL COMMENT '最后提现时间',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- 插入默认支付方式
INSERT INTO payment_methods (id, name, code, description, icon_url, sort_order, is_active, config) VALUES
...
-- 为现有用户创建钱包
INSERT INTO user_wallets (id, user_id, balance, total_recharge, created_at)
SELECT 
...
-- 插入示例充值记录
INSERT INTO recharge_records (id, recharge_number, user_id, wallet_id, amount, payment_method_code, payment_status, paid_amount, paid_time, description) VALUES
...
-- 插入示例钱包交易记录
INSERT INTO wallet_transactions (id, wallet_id, user_id, transaction_type, amount, balance_before, balance_after, related_id, related_type, description, status) VALUES
...
-- 插入示例退款申请
INSERT INTO refund_applications (id, order_id, user_id, amount, reason, status, created_at) VALUES
